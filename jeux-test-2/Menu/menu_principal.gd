...
func init_save_if_needed():
	if not FileAccess.file_exists(save_path):              # vérifie si un fichier de sauvegarde existe déjà
		...
		if file:                                                         # vérifie que l'ouverture a réussi
			file.store_string(JSON.stringify(default_data, "\t"))         # écrit les données par défaut en JSON indenté
			file.close()                                                 # ferme le fichier pour valider l'écriture


# --- MÉDAILLES : AFFICHAGE PAR DÉFAUT (pointillés) ---
func apply_default_medals():
	var pointiller = preload("res://base de donné image/kenney_platformer-art-pixel-redux/Tiles/tile_0076 pointiller.png") # texture vide (pointillés) utilisée pour toutes les médailles non obtenues
	bronze_sprite.texture    = pointiller # médaille bronze 1-1 vide
	argent_sprite.texture    = pointiller # médaille argent 1-1 vide
	gold_sprite.texture      = pointiller # médaille or 1-1 vide
...
func save_game(data):
	...
	file.store_string(JSON.stringify(data, "\t"))             # écrit les données en JSON indenté pour la lisibilité
...
func _ready() -> void:
	...
	$"Level 1/VBoxContainer/Level 1-2".update_visual()   # met à jour la couleur du bouton 1-2 selon les médailles sauvegardées
	...
	# Médailles niveau 1-2
	if data.get("level_1_2_done", false):    # si le niveau 1-2 a été complété au moins une fois
		bronze_sprite_12.texture = preload("res://base de donné image/kenney_platformer-art-pixel-redux/Tiles/tile_0076.png")
	if data.get("level_1_2_silver", false):  # si la médaille argent a été obtenue sur le niveau 1-2
...
func _on_chose_your_level_pressed() -> void:
	$"Level 1/VBoxContainer/Level 1-1".update_visual() # rafraîchit la couleur du bouton 1-1 selon la sauvegarde actuelle
	$"Level 1/VBoxContainer/Level 1-2".update_visual() # rafraîchit la couleur du bouton 1-2 selon la sauvegarde actuelle
	#print("test")
...
func _on_back_pressed() -> void:
	$"Level 1/VBoxContainer/Level 1-1".update_visual() # rafraîchit la couleur du bouton 1-1 avant de revenir au menu
	$"Level 1/VBoxContainer/Level 1-2".update_visual() # rafraîchit la couleur du bouton 1-2 avant de revenir au menu
...
# --- VISIBILITÉ DES BOUTONS SELON PROGRESSION ---
func _update_level_visibility() -> void:
	var data = load_save()                                          # charge la sauvegarde pour lire la progression
	var lvl11 = $"Level 1/VBoxContainer/Level 1-1"
	var lvl12 = $"Level 1/VBoxContainer/Level 1-2"
	var lvl13 = $"Level 1/VBoxContainer/Level 1-3"

	lvl12.visible = data.get("level_1_1_done", false)              # 1-2 visible seulement si 1-1 terminé
	lvl13.visible = data.get("level_1_2_done", false)              # 1-3 visible seulement si 1-2 terminé

	# Recalcule la navigation focus selon les boutons visibles
	if not lvl12.visible and not lvl13.visible:
		lvl11.focus_neighbor_bottom = lvl11.get_path_to($Reset)    # 1-1 → Reset directement
		$Reset.focus_neighbor_top   = $Reset.get_path_to(lvl11)
	elif not lvl12.visible:
		lvl11.focus_neighbor_bottom = lvl11.get_path_to(lvl13)     # 1-1 saute directement vers 1-3
		lvl13.focus_neighbor_top    = lvl13.get_path_to(lvl11)
		lvl13.focus_neighbor_bottom = lvl13.get_path_to($Reset)
		$Reset.focus_neighbor_top   = $Reset.get_path_to(lvl13)
	else:
		lvl11.focus_neighbor_bottom = lvl11.get_path_to(lvl12)     # navigation normale 1-1 → 1-2 → 1-3
		lvl12.focus_neighbor_top    = lvl12.get_path_to(lvl11)
		lvl12.focus_neighbor_bottom = lvl12.get_path_to(lvl13 if lvl13.visible else $Reset)
		if lvl13.visible:
			lvl13.focus_neighbor_top    = lvl13.get_path_to(lvl12)
			lvl13.focus_neighbor_bottom = lvl13.get_path_to($Reset)
			$Reset.focus_neighbor_top   = $Reset.get_path_to(lvl13)
		else:
			$Reset.focus_neighbor_top   = $Reset.get_path_to(lvl12)
...
func _on_level_1_pressed() -> void:
	...
	$"Level 1/VBoxContainer/Level 1-2".update_visual() # rafraîchit la couleur du bouton 1-2
	_update_level_visibility()                         # met à jour quels niveaux sont accessibles selon la progression
	if box.visible:
...
func _on_reset_pressed() -> void:
	var default_data = {                      # construit le dictionnaire de sauvegarde par défaut à écrire
		...
		"level_1_2_silver": false,            # réinitialise la médaille argent du niveau 1-2
		"level_1_2_gold": false,              # réinitialise la médaille or du niveau 1-2
		"piece_objectif": {                   # réinitialise toutes les pièces objectif
			...
			"Lvl_1_2": {                      # sous-dictionnaire pour le niveau 1-2
				"piece_objectif_1": false,    # remet la pièce 1 à non collectée
				"piece_objectif_2": false,    # remet la pièce 2 à non collectée
				"piece_objectif_3": false,    # remet la pièce 3 à non collectée
				"piece_objectif_4": false,    # remet la pièce 4 à non collectée
			...
			"Lvl_1_3": {                      # sous-dictionnaire pour le niveau 1-2
				"piece_objectif_1": false,    # remet la pièce 1 à non collectée
				"piece_objectif_2": false,    # remet la pièce 2 à non collectée
				"piece_objectif_3": false,    # remet la pièce 3 à non collectée
				"piece_objectif_4": false,    # remet la pièce 4 à non collectée
	...
	var file = FileAccess.open(save_path, FileAccess.WRITE)      # ouvre le fichier de sauvegarde en écriture
	file.store_string(JSON.stringify(default_data, "\t"))         # écrase le fichier avec les données par défaut en JSON indenté
	file.close()                                                 # ferme le fichier pour valider l'écriture
	...
	_update_level_visibility()       # recache les boutons 1-2 et 1-3 puisque la progression est réinitialisée
